...
function registerUser($nom, $prenom, $email, $tel, $password, $role)
{
    global $pdo;

    // Check if the signup-profile-pic field is set in $_FILES
    $hashedPassword = password_hash($password, PASSWORD_DEFAULT);

    // Insert User data into the database
    $stmt = $pdo->prepare("INSERT INTO Users (Nom, Prenom, Email, Tel, PasswordU, UserRole) VALUES (?, ?, ?, ?, ?, 'User')");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["signup"])) {
    ...
    registerUser($nom, $prenom, $email, $tel, $password, $role);
}
?>
<!DOCTYPE html>
<html lang="en">
